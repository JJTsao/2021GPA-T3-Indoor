...
void main()
{
	float visibility = max(0.2, textureProj(shadow_tex, shadow_coord));
	...
	if(obj_id == 1 && normal_mapping_flag == 1) { // normal mapping (tangent space)
		...
		float attenuation = min( 1.0 / (1 + pow(d, 2)), 1.0);
		// attenuation = max( 1.0 / (0.01 * pow(d, 2)), 2.0);
		...
		if(point_light_enabled == 1) fragColor += CalcPointLight(point_L, N, V, attenuation, Kd);
		...
		if(point_light_enabled == 1) fragColor += CalcPointLight(point_L, N, V, attenuation, diffuse_Kd);
	} else if(obj_id == 2 && point_light_enabled == 1) { // Point Light Sphere
	...
	} else if(obj_id == 2 && point_light_enabled == 0) {
		discard;
	} else {
		vec3 dir_L = normalize(dirLightDir);
		vec3 point_L = normalize(pointLightDir);
		float d = length(pointLightDir);
		float attenuation = min( 1.0 / (1 + pow(d, 2)), 1.0);
		// attenuation = min( 1.0 / (0.01 * pow(d, 2)), 2.0);
